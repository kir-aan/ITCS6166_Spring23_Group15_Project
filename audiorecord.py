...
def main():
    st.title("Audio Driven Talking Head")
    with st.sidebar:
        imge=image_select(label="Choose a Talking Head", 
                images=[
                "examples/obama.jpg",
                "examples/hermione.jpg",
                "examples/john.jpg",
                "examples/dragonmom.jpg",
                ],
                captions=["Mr Obama", "Hermione", "John", "Daenerys"],
                )
    
    def recorder_factory():
        return MediaRecorder("examples/record.wav")
    ...
    def run_model(imge):
        ...
        parser.add_argument('--jpg', type=str, default=p)
        ...
        au_data = []
        au_emb = []
        ains = glob.glob1('examples', 'record.wav')
    ...
    if 'record.wav' in ains:
        run_model(imge)
        with col2:
            for ain in ains:
                i=imge.split('/')[1]
                v=i.split('.')[0]
                OUTPUT_MP4_NAME = '{}_pred_fls_record_audio_embed.mp4'.format(v)
            st.write("Chosen Image | Talking Head | Scaled Video")
            video_file = open('examples/{}'.format(OUTPUT_MP4_NAME),'rb')
            video_bytes = video_file.read()
            st.video(video_bytes)
        os.remove('examples/record.wav')
